...
### Created by Peter Forret ( pforret ) on 2022-10-24
### Based on https://github.com/pforret/bashew 1.19.0
script_version="0.0.1" # if there is a VERSION.md in this script's folder, it will take priority for version number
readonly script_author="peter@forret.com"
readonly script_created="2022-10-24"
...
option|n|number|number of times to repeat the command|1000
option|w|wait|seconds to wait between repeating the command|5
choice|1|action|action to perform|run,file,check,env,update
...
do_run() {
    ...
    # shellcheck disable=SC2154
    user=$(whoami)
    id=$(echo "$user@$HOSTNAME $command" | Str:digest 10)
    IO:debug "ID for this command: $id"
    # shellcheck disable=SC2154
    tmp_output="$tmp_dir/$script_prefix.$id.compare.txt"
    # shellcheck disable=SC2154
    [[ $erase -gt 0 ]] && rm -f "$tmp_output"
    [[ ! -f "$tmp_output" ]] && touch "$tmp_output"
    hash_output=$(< "$tmp_output" Str:digest 16)

    tmp_latest="$tmp_dir/$script_prefix.$id.latest.txt"
    # shellcheck disable=SC2154
    while [[ $i -lt $number ]] ; do
      i=$((i + 1))
      IO:progress "Run [$command] (#$i/$number @ $SECONDS secs)"
      eval "$command &> '$tmp_latest'"
      hash_latest=$(< "$tmp_latest" Str:digest 16)
      if [[ ! "$hash_latest" == "$hash_output" ]] ; then
        tput bel
        echo "   "
        IO:success "### Output changed at $(date)"
        if [[ $diff -eq 0 ]] ; then
          IO:success "### NEW OUTPUT"
          cat "$tmp_latest"
        else
          IO:success "### OUTPUT DIFF"
      ...
      IO:progress "Ran [$command] (#$i/$number @ $SECONDS secs) - wait $wait seconds"
      sleep "$wait"
    done
}
...
do_file() {
    ...
    file="$command"
    IO:log "file [$file]"
    id=$(echo "$user@$HOSTNAME $file" | Str:digest 10)
    IO:debug "ID for this file: $id"
    tmp_output="$tmp_dir/$script_prefix.$id.compare.txt"
    # shellcheck disable=SC2154
    [[ $erase -gt 0 ]] && rm -f "$tmp_output"
    ...
    while [[ $i -lt $number ]] ; do
      ...
      if [[ ! "$hash_latest" == "$hash_output" ]] ; then
        tput bel
        echo "   "
        IO:success "### File changed at $(date)"
        if [[ $diff -eq 0 ]] ; then
          IO:success "### NEW FILE"
          cat "$file"
        else
          IO:success "### FILE DIFF"
      ...
      IO:progress "Check [$command] (#$i/$number @ $SECONDS secs) - wait $wait seconds"
      sleep "$wait"
    done
}
